...
class dbHelper {
    ...
    function insert($table, $columnsArray, $requiredColumnsArray) {
        ...
        try{
			...
			if(is_array($columnsArray->dist)){
				if(count($columnsArray->dist) > 0){
					$columnsArray->Directors = implode(",",$columnsArray->dist);				
				}else{
					$columnsArray->Directors = '';
				}
			}
			unset($columnsArray->dist);
			unset($columnsArray->dir_name);				
